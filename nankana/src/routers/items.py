...
from src.schemas import ItemCreate, ShowItem
...
async def retrieve_item_by_id(id: int, db: Session = Depends(get_db)):
    ...
    if not item:
        ...
        raise HTTPException(
            status_code=code, detail=f"Item id({id}) does not Exist")
    return item
...
async def token_decode(token, db: get_db):
    ...
    try:
        ...
        if username is None:
            ...
            raise HTTPException(
                status_code=code, detail=f"Unable to verify credentials")

        user = db.query(User).filter(User.email == username).first()
        if not user:
            code: int = status.HTTP_401_UNAUTHORIZED
            raise HTTPException(
                status_code=code, detail=f"User does not Exists")
        return user
    except Exception as e:
        logging.exception(e)
